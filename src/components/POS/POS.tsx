...
export function POS({ user, theme }: { user: any, theme: 'light' | 'dark' }) {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        cart.forEach(item => {
          ...
          if (item.category === 'Flavor' || (item.drinkId === 'calamansi' && item.flavor === 'Mango')) {
            ...
            deductions[flavorName] = (deductions[flavorName] || 0) + item.quantity;
          }
                ...
                {Object.keys(selectedDrink.sizes).map(size => {
                  ...
                  return (
                    <button
                      ...
                      className="group h-48 rounded-[48px] border-2 border-stone-100 dark:border-stone-800 bg-white dark:bg-stone-900 flex flex-col items-center justify-center transition-all hover:border-lemon hover:shadow-2xl hover:shadow-lemon/5 active:scale-95"
                    >
                      ...
                      <p className="text-[11px] font-black text-stone-400 uppercase tracking-[0.4em] mt-4">{data.type}</p>
            ...
            >
              ...
              <div className="space-y-12">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                      {(options as string[]).map((opt: string) => (
                        <button
                          ...
                          className={cn(
                            "p-8 rounded-[40px] border-2 transition-all active:scale-95 text-center group",
                            selectedFlavor === opt 
                        ...
                        >
                          <p className="text-sm font-black uppercase tracking-widest leading-none">{opt}</p>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {!selectedDrink?.sizes?.[selectedSize]?.categories && selectedDrink?.sizes?.[selectedSize]?.flavors && (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    {selectedDrink.sizes[selectedSize].flavors.map((opt: string) => (
                      <button
                        key={opt}
                        onClick={() => {
                ...
                {ADDONS.map(addon => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "px-10 py-6 rounded-[32px] border-2 transition-all active:scale-95 text-xs font-black uppercase tracking-widest flex items-center gap-4 shadow-sm",
                        isSelected 
        ...
        <div className="p-8 bg-stone-50 dark:bg-stone-800/30 border-t border-stone-200 dark:border-stone-800 space-y-6 relative overflow-hidden transition-colors">
          ...
          <motion.button 
            ...
            animate={cart.length > 0 ? {
              boxShadow: theme === 'dark' ? ["0px 20px 40px rgba(0, 0, 0, 0.4)", "0px 20px 60px rgba(0, 0, 0, 0.6)", "0px 20px 40px rgba(0, 0, 0, 0.4)"] : ["0px 20px 40px rgba(61, 82, 59, 0.3)", "0px 20px 60px rgba(61, 82, 59, 0.5)", "0px 20px 40px rgba(61, 82, 59, 0.3)"]
            } : {}}
            transition={{ 
