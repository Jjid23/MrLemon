...
export function Layout({ children, user, permissions = [], view, setView, theme, setTheme }: LayoutProps) {
      ...
      <aside className="w-20 md:w-72 bg-white dark:bg-stone-900 border-r border-stone-200 dark:border-stone-800 flex flex-col items-center py-10 z-20 shadow-xl shadow-stone-200/20 dark:shadow-none relative transition-colors duration-300">
        ...
        <div className="flex items-center gap-4 mb-16 px-8 w-full group cursor-default">
          <div className="w-12 h-12 bg-lemon rounded-3xl flex items-center justify-center text-stone-800 shrink-0 shadow-lg shadow-lemon/20 transform group-hover:rotate-12 transition-transform duration-500">
              <img src="/Mrlemonlogo.jpg" alt="Mr Lemon" className="w-full h-full object-cover p-1" />
            </div>
        ...
        <div className="w-full px-4 space-y-6">
          <button
            ...
            className="w-full flex items-center justify-center md:justify-start gap-4 px-5 py-4 text-stone-400 dark:text-stone-500 hover:text-lemon dark:hover:text-lemon hover:bg-lemon/5 rounded-[28px] transition-all group active:scale-95"
          >
            <div className="w-12 h-12 bg-stone-50 dark:bg-stone-800 group-hover:bg-lemon/10 rounded-2xl flex items-center justify-center transition-colors">
              {theme === 'dark' ? <Sun size={24} /> : <Moon size={24} />}
            </div>
            <span className="hidden md:block font-black text-sm uppercase tracking-widest">{theme === 'dark' ? 'Light Mode' : 'Dark Mode'}</span>
          </button>
