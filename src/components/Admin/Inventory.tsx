...
import { Package, Plus, Search, Trash2, Edit3, Save, X, AlertCircle } from 'lucide-react';
...
export function Inventory() {
  ...
  const [newItem, setNewItem] = useState({ name: '', quantity: 0, unit: 'pcs', lowStockThreshold: 10 });
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsAdding(false);
      setNewItem({ name: '', quantity: 0, unit: 'pcs', lowStockThreshold: 10 });
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-stone-50/50 dark:bg-stone-950 border-b border-stone-100 dark:border-stone-800">
              ...
              <th className="px-10 py-8 text-[11px] font-black uppercase tracking-[0.4em] text-stone-500 dark:text-stone-600 italic">Status</th>
          ...
          <tbody>
            {isAdding && (
              <tr className="border-b border-lemon/20 bg-lemon/5 animate-in fade-in slide-in-from-top-4">
                ...
                <td className="px-10 py-8">
                  <div className="bg-white/50 dark:bg-stone-800 border border-stone-100 dark:border-stone-700 rounded-full px-6 py-2 inline-block">
                    <span className="text-[11px] font-black uppercase tracking-[0.5em] text-stone-300 dark:text-stone-700 italic">Provisioning</span>
                  </div>
                </td>
            ...
            {items.map(item => (
              <tr key={item.id} className="border-b border-stone-50 dark:border-stone-800 hover:bg-stone-50/50 dark:hover:bg-stone-800/30 transition-all group">
                ...
                <td className="px-10 py-8">
                   <span className="font-black text-3xl text-stone-800 dark:text-stone-100 tracking-tighter italic">{item.quantity}</span>
                   <span className="ml-3 text-[11px] font-black uppercase tracking-[0.3em] text-stone-400 dark:text-stone-600">{item.unit}</span>
                </td>
                <td className="px-10 py-8">
                   {item.quantity <= item.lowStockThreshold ? (
                     <div className="inline-flex items-center gap-3 bg-red-50 dark:bg-red-900/20 text-red-500 px-6 py-2 rounded-full border border-red-100 dark:border-red-900/30">
                       <AlertCircle size={16} strokeWidth={2.5} />
                       <span className="font-black text-[11px] uppercase tracking-[0.3em] italic">Low Stock</span>
                     </div>
                   ) : (
                     <div className="inline-flex items-center gap-3 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 px-6 py-2 rounded-full border border-emerald-100 dark:border-emerald-900/30">
                       <span className="font-black text-[11px] uppercase tracking-[0.3em] italic">In Stock</span>
                     </div>
                   )}
                </td>
                <td className="px-10 py-8 text-right">
